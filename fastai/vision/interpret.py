...
class SegmentationInterpretation(Interpretation):
    ...
    def __init__(
        ...
        super(SegmentationInterpretation, self).__init__(
    ...
    def _interp_show(
        ...
        for i, l in enumerate([self.i2c[l] for l in np_im_labels]):
            ...
            axes[1].text(
                mod, div, f"{l}", ha="center", color="white", fontdict={"size": sz}
            )
    ...
    def show_xyz(self, i, classes: list = None, sz=10):
        ...
        self._interp_show(
            ImageSegment(self.y_true[i]), classes, sz=sz, title_suffix="true"
        )
        self._interp_show(
            ImageSegment(self.pred_class[i][None, :]),
            classes,
            sz=sz,
            title_suffix="pred",
        )
    ...
    def _generate_confusion(self):
        ...
        for c_j in range(self.data.c):
            ...
            for c_i in range(self.data.c):
                ...
                total_intersect = (
                    (true_binary * pred_binary).view(n, -1).sum(dim=1).float()
                )
        ...
        self.single_img_cm = to_np(
            torch.stack(single_img_confusion)
            .permute(1, 0)
            .view(-1, self.data.c, self.data.c)
        )
        self.mean_cm = to_np(
            torch.tensor(mean_confusion).view(self.data.c, self.data.c)
        )
...
class ObjectDetectionInterpretation(Interpretation):
    ...
    def __init__(
        ...
        super(ObjectDetectionInterpretation, self).__init__(
            learn, preds, y_true, losses, ds_type
        )
