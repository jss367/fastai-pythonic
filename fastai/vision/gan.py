...
def basic_critic(
    ...
    **conv_kwargs,
    ...
    layers = [
        conv_layer(
            n_channels, n_features, 4, 2, 1, leaky=0.2, norm_type=None, **conv_kwargs
        )
    ]  # norm_type=None?
    ...
    while cur_size > 4:
        layers.append(
            conv_layer(cur_ftrs, cur_ftrs * 2, 4, 2, 1, leaky=0.2, **conv_kwargs)
        )
        cur_ftrs *= 2
        cur_size //= 2
...
def basic_generator(
    ...
    **conv_kwargs,
    ...
    while cur_size < in_size // 2:
        layers.append(
            conv_layer(cur_ftrs, cur_ftrs // 2, 4, 2, 1, transpose=True, **conv_kwargs)
        )
        cur_ftrs //= 2
        cur_size *= 2
    layers += [
...
class GANLoss(GANModule):
    ...
    def critic(self, real_pred, input):
        ...
        fake = self.gan_model.generator(input.requires_grad_(False)).requires_grad_(
            True
        )
...
class GANTrainer(LearnerCallback):
    ...
    def on_train_begin(self, **kwargs):
        ...
        self.smoothenerG, self.smoothenerC = (
            SmoothenValue(self.beta),
            SmoothenValue(self.beta),
        )
    ...
    def on_epoch_end(self, pbar, epoch, last_metrics, **kwargs):
        ...
        return add_metrics(
            last_metrics,
            [
                getattr(self.smoothenerG, "smooth", None),
                getattr(self.smoothenerC, "smooth", None),
            ],
        )
...
class GANLearner(Learner):
    ...
    def __init__(
        ...
        **learn_kwargs,
        ...
        super().__init__(
            data, gan, loss_func=loss_func, callback_fns=[switcher], **learn_kwargs
        )
        trainer = GANTrainer(
            self, clip=clip, switch_eval=switch_eval, show_img=show_img
        )
