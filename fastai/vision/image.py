...
class Image(ItemBase):
    ...
    def apply_tfms(
        ...
        default_rsz = (
            ResizeMethod.SQUISH
            if (size is not None and is_listy(size))
            else ResizeMethod.CROP
        )
        ...
        for tfm in tfms:
            if tfm.tfm in xtra:
                ...
                if resize_method in (ResizeMethod.CROP, ResizeMethod.PAD):
                    x = tfm(
                        x,
                        size=_get_crop_target(size, mult=mult),
...
class ImagePoints(Image):
    ...
    def data(self) -> Tensor:
        ...
        if self.transformed:
            if (
                "remove_out" not in self.sample_kwargs
                or self.sample_kwargs["remove_out"]
            ):
...
class ImageBBox(ImagePoints):
    ...
    def __init__(
        ...
        if (
            labels is not None
            and len(labels) > 0
            and not isinstance(labels[0], Category)
        ):
...
def show_image(
    ...
    **kwargs,
    ...
    ax.imshow(image2np(img.data), **xtr) if (hasattr(img, "data")) else ax.imshow(
        img, **xtr
    )
    if hide_axis:
        ax.axis("off")
        ...
        **kwargs: Any,
        ...
        if args:
            ...
            return RandTransform(
                self, kwargs=kwargs, is_random=is_random, use_on_y=use_on_y, p=p
            )
...
def _affine_grid(size: TensorImageSize) -> FlowField:
    ...
    grid[:, :, :, 0] = torch.ger(torch.ones(H), linear_points).expand_as(
        grid[:, :, :, 0]
    )
    linear_points = torch.linspace(-1, 1, H) if H > 1 else tensor([-1.0])
    grid[:, :, :, 1] = torch.ger(linear_points, torch.ones(W)).expand_as(
        grid[:, :, :, 1]
    )
