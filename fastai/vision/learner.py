...
def create_cnn_model(base_arch: Callable,
                     nc: int,
                     cut: Union[int,
                                Callable] = None,
...
def _cl_int_from_learner(
    ...
    preds = learn.TTA(
        ds_type=ds_type,
        with_loss=True) if tta else learn.get_preds(
        ds_type=ds_type,
        activ=activ,
        with_loss=True)
...
def _cl_int_plot_multi_top_losses(self, samples: int = 3, figsize: Tuple[int, int] = (
        8, 8), save_misclassified: bool = False):
    ...
    for i, pred in enumerate(predclass):
        ...
        if l_dim > 1:
            infotup = (i, pred, where_truth, losses[i][pred], np.round(self.preds[i], decimals=3)[pred], mismatch)
        else:
            infotup = (i, pred, where_truth, losses[i], np.round(self.preds[i], decimals=3)[pred], mismatch)
