...
class CycleGanLoss(Module):
    ...
    def forward(self, output, target):
        ...
        idt_loss = self.l_idt * (
            self.l_B * F.l1_loss(idt_A, self.real_B)
            + self.l_A * F.l1_loss(idt_B, self.real_A)
        )
        # Generators are trained to trick the critics so the following should be ones
        gen_loss = self.crit(self.cgan.D_A(fake_A), True) + self.crit(
...
class CycleGANTrainer(LearnerCallback):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        return add_metrics(
            last_metrics, [s.smooth for k, s in self.smootheners.items()]
        )
