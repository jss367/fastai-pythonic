...
def _crop_pad(
    ...
    f_crop_pad = (
        _crop_pad_image_points if isinstance(x, ImagePoints) else _crop_pad_default
    )
...
def _tilt(c, direction: uniform_int, magnitude: uniform = 0, invert=False):
    ...
    coeffs = (
        _find_coeffs(targ_pts, _orig_pts)
        if invert
        else _find_coeffs(_orig_pts, targ_pts)
    )
...
def get_transforms(
    ...
    if max_lighting:
        res.append(
            brightness(
                change=(0.5 * (1 - max_lighting), 0.5 * (1 + max_lighting)),
                p=p_lighting,
            )
        )
        res.append(
