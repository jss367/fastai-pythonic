...
class XResNet(nn.Sequential):
    def __init__(self, expansion, layers, c_in=3, c_out=1000):
        ...
        blocks = [
            self._make_layer(
                expansion, block_szs[i], block_szs[i + 1], l, 1 if i == 0 else 2
            )
...
xresnet18_deep = partial(
    xresnet, expansion=1, n_layers=[2, 2, 2, 2, 1, 1], name="xresnet18_deep"
)
xresnet34_deep = partial(
    xresnet, expansion=1, n_layers=[3, 4, 6, 3, 1, 1], name="xresnet34_deep"
)
xresnet50_deep = partial(
    xresnet, expansion=4, n_layers=[3, 4, 6, 3, 1, 1], name="xresnet50_deep"
)
