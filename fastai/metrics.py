...
class ConfusionMatrix(Callback):
    ...
    def on_batch_end(self, last_output: Tensor, last_target: Tensor, **kwargs):
        ...
        if self.cm is None:
            self.cm = torch.zeros(
                (self.n_classes, self.n_classes), device=torch.device("cpu")
            )
...
class KappaScore(ConfusionMatrix):
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.weights is None:
            ...
            w = (
                torch.abs(w - torch.t(w))
                if self.weights == "linear"
                else (w - torch.t(w)) ** 2
            )
        else:
            raise ValueError(
                'Unknown weights. Expected None, "linear", or "quadratic".'
            )
...
class MultiLabelFbeta(Callback):
    ...
    def on_batch_end(self, last_output, last_target, **kwargs):
        pred, targ = (
            (
                (last_output.sigmoid() if self.sigmoid else last_output) > self.thresh
            ).byte(),
            last_target.byte(),
        )
        m = pred * targ
    ...
    def on_epoch_end(self, last_metrics, **kwargs):
        ...
        if self.average == "micro":
            ...
            res = self.fbeta_score(
                (self.tp / self.total_pred), (self.tp / self.total_targ)
            ).mean()
        elif self.average == "weighted":
            scores = self.fbeta_score(
                (self.tp / self.total_pred), (self.tp / self.total_targ)
            )
            ...
            raise Exception(
                "Choose one of the average types: [micro, macro, weighted, none]"
            )
