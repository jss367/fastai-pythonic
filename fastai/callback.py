...
class OptimWrapper:
    ...
    @classmethod
    def load_with_state_and_layer_group(
...
class Callback:
    ...
    def __repr__(self):
        ...
        list_repr = [self.__class__.__name__] + [
            f"{k}: {getattr(self, k)}"
            for k in attrs
            if k != "self" and k not in to_remove
        ]
        return "\n".join(list_repr)
...
class CallbackHandler:
    ...
    def __post_init__(self) -> None:
        ...
        self.metrics = [
            (met if isinstance(met, Callback) else AverageMetric(met))
    ...
    def on_batch_begin(
        self, xb: Tensor, yb: Tensor, train: bool = True
    ) -> Tuple[Any, Any]:
