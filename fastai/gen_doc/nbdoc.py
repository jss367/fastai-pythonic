...
def show_doc(
    ...
    test_link, test_modal = (
        get_pytest_html(elt, anchor_id=anchor_id) if show_tests else ("", "")
    )
...
def format_docstring(
    ...
    args = (
        inspect.getfullargspec(elt).args
        if not is_enum(elt.__class__)
        else elt.__members__.keys()
    )
...
def get_pytorch_link(ft) -> str:
    ...
    if name.startswith("torch.nn") and inspect.ismodule(
        ft
