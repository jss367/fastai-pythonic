...
def _decode(df):
    return np.array(
        [[df.columns[i] for i, t in enumerate(x) if t == 1] for x in df.values],
        dtype=np.object,
    )
...
class ItemList:
    ...
    def new(
        self, items: Iterator, processor: PreProcessors = None, **kwargs
    ) -> "ItemList":
    ...
    def use_partial_data(
        self, sample_pct: float = 0.01, seed: int = None
    ) -> "ItemList":
    ...
    def split_by_folder(
        self, train: str = "train", valid: str = "valid"
    ) -> "ItemLists":
        "Split the data depending on the folder (`train` or `valid`) in which the filenames are."
        return self.split_by_idxs(
            self._get_by_folder(train), self._get_by_folder(valid)
        )
    ...
    def split_subsets(
        self, train_size: float, valid_size: float, seed=None
    ) -> "ItemLists":
    ...
    def split_by_files(self, valid_names: "ItemList") -> "ItemLists":
        ...
        if isinstance(self.items[0], Path):
            ...
            return self.split_by_valid_func(
                lambda o: os.path.basename(o) in valid_names
            )

    def split_by_fname_file(
        self, fname: PathOrStr, path: PathOrStr = None
    ) -> "ItemLists":
    ...
    def split_from_df(self, col: IntsOrStrs = 2):
        ...
        valid_idx = np.where(
            self.inner_df.iloc[:, df_names_to_idx(col, self.inner_df)]
        )[0]
        return self.split_by_idx(valid_idx)
...
It's either because your data source is empty or because your labelling function raised an error."""
    ...
    def label_from_df(self, cols: IntsOrStrs = 1, label_cls: Callable = None, **kwargs):
        ...
        return self._label_from_list(
            _maybe_squeeze(labels), label_cls=label_cls, **kwargs
        )

    def label_const(
        self, const: Any = 0, label_cls: Callable = None, **kwargs
    ) -> "LabelList":
    ...
    def label_from_func(
        self, func: Callable, label_cls: Callable = None, **kwargs
    ) -> "LabelList":
        "Apply `func` to every input to get its label."
        return self._label_from_list(
            [func(o) for o in self.items], label_cls=label_cls, **kwargs
        )

    def label_from_folder(self, label_cls: Callable = None, **kwargs) -> "LabelList":
        "Give a label to each filename depending on its folder."
        return self.label_from_func(
            func=lambda o: (o.parts if isinstance(o, Path) else o.split(os.path.sep))[
                -2
            ],
            label_cls=label_cls,
            **kwargs,
...
class ItemLists:
    ...
    def label_from_lists(
        self,
        train_labels: Iterator,
        valid_labels: Iterator,
        label_cls: Callable = None,
        **kwargs,
    ) -> "LabelList":
        ...
        self.train = self.train._label_list(
            x=self.train, y=label_cls(train_labels, **kwargs)
        )
    ...
    def databunch(self, **kwargs):
        ...
        raise Exception(
            "Your data isn't labeled, can't turn it into a `DataBunch` yet!"
        )
...
class LabelLists(ItemLists):
    ...
    def get_processors(self):
        ...
        procs_x, procs_y = (
            listify(self.train.x._processor),
            listify(self.train.y._processor),
        )
    ...
    def databunch(
        ...
        **kwargs,
        ...
        if getattr(self, "normalize", False):  # In case a normalization was serialized
            norm = self.normalize
            data.normalize(
                (norm["mean"], norm["std"]), do_x=norm["do_x"], do_y=norm["do_y"]
            )
    ...
    def add_test(self, items: Iterator, label: Any = None, tfms=None, tfm_y=None):
        ...
        if isinstance(items, MixedItemList):
            items = self.valid.x.new(
                items.item_lists, inner_df=items.inner_df
            ).process()
    ...
    def add_test_folder(
        self, test_folder: str = "test", label: Any = None, tfms=None, tfm_y=None
    ):
...
def _check_kwargs(ds: ItemList, tfms: TfmList, **kwargs):
    ...
    if len(ds.items) >= 1:
        ...
        try:
            ...
            raise Exception(
                f"It's not possible to apply those transforms to your dataset:\n {e}"
            )
...
class LabelList(Dataset):
    ...
    def to_df(self) -> None:
        ...
        return pd.DataFrame(
            dict(x=self.x._relative_item_paths(), y=[str(o) for o in self.y])
        )
    ...
    def load_state(cls, path: PathOrStr, state: dict) -> "LabelList":
        ...
        res = cls(
            x, y, tfms=state["tfms"], tfm_y=state["tfm_y"], **state["tfmargs"]
        ).process()
    ...
    def transform_y(self, tfms: TfmList = None, **kwargs):
        ...
        tfms_y = list(
            filter(
                lambda t: getattr(t, "use_on_y", True),
                listify(self.tfms if tfms is None else tfms),
            )
        )
...
class MixedItemList(ItemList):
    def __init__(
        ...
        if processor is None:
            default_procs = [
                [p(ds=il) for p in listify(il._processor)] for il in item_lists
            ]
