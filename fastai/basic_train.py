...
def _loss_func2activ(loss_func):
    ...
    if cls_name in loss_func_name2activ:
        if cls_name == "poisson_nll_loss" and (
            not getattr(loss_func, "log_input", True)
        ):
            return
...
class Learner:
    ...
    def export(self, file: PathLikeOrBinaryStream = "export.pkl", destroy=False):
        ...
        with ModelOnCPU(self.model) as m:
            state["model"] = m
            xtra = (
                dict(normalize=self.data.norm.keywords)
                if getattr(self.data, "norm", False)
                else {}
            )
    ...
    def save(
        ...
        if rank_distrib():
            return  # don't save if slave proc
        target = (
            self.path / self.model_dir / f"{file}.pth" if is_pathlike(file) else file
        )
    ...
    def predict(
        ...
        **kwargs,
        ...
        y = (
            ds.y.reconstruct(pred, x)
            if has_arg(ds.y.reconstruct, "x")
            else ds.y.reconstruct(pred)
        )
...
class Recorder(LearnerCallback):
    ...
    def on_train_begin(
        ...
        self.names = (
            ["epoch", "train_loss"]
            if self.no_val
            else ["epoch", "train_loss", "valid_loss"]
        )
    ...
    def plot_losses(
        ...
        val_iter = self._split_list_val(
            np.cumsum(self.nb_batches), skip_start, skip_end
        )
