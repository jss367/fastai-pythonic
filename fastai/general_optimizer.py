...
class AvgStatistic(Statistic):
    ...
    def update(self, state, param, val=None, step=None):
        ...
        # For everything else, `state` is a scalar
            ...
            res = state * param + self._get_val1(val) * (
                1 - param if self.decay else 1.0
            )
        elif self.count != 0:
            res = state * param + self.val / self.count * (
                1 - param if self.decay else 1.0
            )
        else:
            return state
...
class AvgSquare(AvgStatistic):
    def __init__(
        ...
        super().__init__(
            name, param=param, scope=scope, init=init, decay=decay, debias=debias
        )
    ...
    def _get_val2(self, state, val, param):
        return (
            state.addcmul_(1 - param, val, val)
            if self.decay
            else state.addcmul_(val, val)
        )
...
class GeneralOptimizer(Optimizer):
    ...
    def _split_stats(self, stats):
        splits = [
            [stat for stat in listify(stats) if stat.scope == scope]
            for scope in StatScope
        ]
        for split, s in zip(
            [splits[0], splits[1], splits[2] + splits[3] + splits[4]], StatScope
        ):
    ...
    def init_stats(self):
        ...
        for i, pg in enumerate(self.param_groups):
            ...
            for p in pg["params"]:
                ...
                self.state[p].update(
                    self._init_stats(
                        self.channel_stats, p.data.view(p.data.size(0), -1).mean(1)
                    )
                )
