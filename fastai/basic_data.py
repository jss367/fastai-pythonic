...
class DataBunch:
    ...
    def __init__(
        ...
        self.single_dl = _create_dl(
            DataLoader(valid_dl.dataset, batch_size=1, num_workers=0)
        )
        self.path = Path(path)
        if not no_check:
            self.sanity_check()
    ...
    def _init_ds(
        ...
        fix_ds = (
            valid_ds.new(train_ds.x, train_ds.y)
            if hasattr(valid_ds, "new")
            else train_ds
        )
    ...
    def create(
        ...
        **dl_kwargs,
        ...
        dls = [
            DataLoader(
                d, b, shuffle=s, drop_last=s, num_workers=num_workers, **dl_kwargs
            )
            for d, b, s in zip(
                datasets, (bs, val_bs, val_bs, val_bs), (True, False, False, False)
            )
            if d is not None
        ]
        return cls(
    ...
    def add_test(
        self, items: Iterator, label: Any = None, tfms=None, tfm_y=None
    ) -> None:
    ...
    def one_item(
        self, item, detach: bool = False, denorm: bool = False, cpu: bool = False
    ):
        "Get `item` into a batch. Optionally `detach` and `denorm`."
        ds = self.single_ds
        with ds.set_item(item):
            return self.one_batch(
                ds_type=DatasetType.Single, detach=detach, denorm=denorm, cpu=cpu
            )
    ...
    def show_batch(
        ...
        **kwargs,
        ...
        if has_arg(self.train_ds.y.reconstruct, "x"):
            ys = [
                self.train_ds.y.reconstruct(grab_idx(y, i), x=x)
    ...
    def export(self, file: PathLikeOrBinaryStream = "export.pkl"):
        ...
        xtra = (
            dict(normalize=self.norm.keywords) if getattr(self, "norm", False) else {}
        )
