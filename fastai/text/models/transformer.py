...
class MultiHeadAttention(Module):
    ...
    def forward(self, x: Tensor, mask: Tensor = None, **kwargs):
        return self.ln(
            x + self.drop_res(self.out(self._apply_attention(x, mask=mask, **kwargs)))
        )
    ...
    def _apply_attention(self, x: Tensor, mask: Tensor = None):
        ...
        wq, wk, wv = map(
            lambda x: x.view(bs, x.size(1), self.n_heads, self.d_head), (wq, wk, wv)
        )
        wq, wk, wv = (
            wq.permute(0, 2, 1, 3),
            wk.permute(0, 2, 3, 1),
        ...
        if mask is not None:
            attn_score = (
                attn_score.float().masked_fill(mask, -float("inf")).type_as(attn_score)
            )
        attn_prob = self.drop_att(F.softmax(attn_score, dim=-1))
        attn_vec = torch.matmul(attn_prob, wv)
        return (
            attn_vec.permute(0, 2, 1, 3).contiguous().contiguous().view(bs, x_len, -1)
        )
...
class DecoderLayer(Module):
    ...
    def __init__(
        ...
        self.ff = feed_forward(
            d_model, d_inner, ff_p=ff_p, act=act, double_drop=double_drop
        )
...
class Transformer(Module):
    ...
    def __init__(
        ...
        self.pos_enc = (
            nn.Embedding(ctx_len, d_model)
            if learned_pos_enc
            else PositionalEncoding(d_model)
        )
    ...
    def forward(self, x):
        ...
        inp = self.drop_emb(
            self.encoder(x) + self.pos_enc(pos)[None]
...
class TransformerXL(Module):
    ...
    def __init__(
        ...
        self.mem_len, self.n_layers, self.d_model, self.mask = (
            mem_len,
            n_layers,
            d_model,
            mask,
    ...
    def reset(self):
        ...
        self.hidden = [
            next(self.parameters()).data.new(0) for i in range(self.n_layers + 1)
        ]
    ...
    def forward(self, x):
        ...
        seq_len = m_len + x_len
        mask = (
            torch.triu(x.new_ones(x_len, seq_len), diagonal=1 + m_len).byte()[
                None, None
            ]
            if self.mask
            else None
