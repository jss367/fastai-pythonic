...
def forget_mult_CPU(
    ...
    prev_h = (
        None if hidden_init is None else hidden_init.unsqueeze(1 if batch_first else 0)
    )
...
class QRNNLayer(Module):
    ...
    def __init__(
        ...
        self.output_gate, self.batch_first, self.backward = (
            output_gate,
            batch_first,
            backward,
        )
    ...
    def forward(self, inp, hid=None):
        ...
        if self.zoneout and self.training:
            mask = dropout_mask(f_gate, f_gate.size(), self.zoneout).requires_grad_(
                False
            )
            f_gate = f_gate * mask
...
class QRNN(Module):
    ...
    def __init__(
        ...
        assert not (
            save_prev_x and bidirectional
        ), "Can't save the previous X with bidirectional."
    ...
    def forward(self, inp, hid=None):
        ...
        for i, layer in enumerate(self.layers):
            inp, h = layer(
                inp, None if hid is None else hid[2 * i if self.bidirectional else i]
            )
            new_hid.append(h)
            if self.bidirectional:
            ...
            if self.dropout != 0 and i < len(self.layers) - 1:
                for o in [inp, inp_bwd] if self.bidirectional else [inp]:
                    o = F.dropout(
                        o, p=self.dropout, training=self.training, inplace=False
                    )
        if self.bidirectional:
