    ...
    def from_ids(
        ...
        **kwargs,
        ...
        src = (
            ...
            else src.label_from_lists(
                train_lbls, valid_lbls, classes=classes, processor=[]
            )
        )
        ...
        if cls == TextLMDataBunch:
            ...
            if label_delim is not None:
                src = src.label_from_df(
                    cols=label_cols, classes=classes, label_delim=label_delim
                )
            else:
    ...
    def from_folder(
        ...
        **kwargs,
        ...
        src = (
            src.label_for_lm()
            if cls == TextLMDataBunch
            else src.label_from_folder(classes=classes)
        )
        if test is not None:
...
class TextClasDataBunch(TextDataBunch):
    ...
    def create(
        ...
        **dl_kwargs,
        ...
        val_bs = ifnone(val_bs, bs)
        collate_fn = partial(
            pad_collate, pad_idx=pad_idx, pad_first=pad_first, backwards=backwards
        )
...
class TokenizeProcessor(PreProcessor):
    ...
    def process(self, ds):
        ds.items = _join_texts(
            ds.items, self.mark_fields, self.include_bos, self.include_eos
        )
        tokens = []
...
class TextList(ItemList):
    ...
    def from_folder(
        ...
        **kwargs,
        ...
        return super().from_folder(
            path=path, extensions=extensions, processor=processor, **kwargs
        )
    ...
    def show_xys(self, xs, ys, max_len: int = 70) -> None:
        ...
        for i, (x, y) in enumerate(zip(xs, ys)):
            txt_x = (
                " ".join(x.text.split(" ")[:max_len]) if max_len is not None else x.text
            )
...
class SPProcessor(PreProcessor):
    ...
    def __init__(
        ...
        self.mark_fields, self.include_bos, self.include_eos = (
            mark_fields,
            include_bos,
            include_eos,
        )
    ...
    def process_one(self, item, join=True):
        if join:
            text = _join_texts(
                [item], self.mark_fields, self.include_bos, self.include_eos
            )[0]
            ...
            with ProcessPoolExecutor(self.n_cpus) as e:
                ds.items = np.array(
                    sum(
                        e.map(
                            self._encode_batch,
                            partition_by_cores(ds.items, self.n_cpus),
                        ),
                        [],
                    )
                )
    ...
    def load(cls, path: PathOrStr, tmp_dir: PathOrStr = "tmp", name: str = "spm"):
        cache_dir = Path(path) / tmp_dir
        return cls(
            sp_model=cache_dir / f"{name}.model", sp_vocab=cache_dir / f"{name}.vocab"
        )
