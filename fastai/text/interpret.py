...
class TextClassificationInterpretation(ClassificationInterpretation):
    ...
    def from_learner(
        ...
        return cls(
            learn,
            *learn.get_preds(
                ds_type=ds_type, activ=activ, with_loss=True, ordered=True
            ),
            ds_type=ds_type,
        )
    ...
    def intrinsic_attention(self, text: str, class_id: int = None):
        ...
        self.model.eval()
        cl = self.model[1](lstm_output + (torch.zeros_like(ids).byte(),))[0].softmax(
            dim=-1
        )
        if class_id is None:
            class_id = cl.argmax()
