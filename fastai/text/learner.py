    ...
    def __init__(
        ...
        **learn_kwargs,
        ...
        is_class = hasattr(data.train_ds, "y") and (
            isinstance(data.train_ds.y, CategoryList)
            or isinstance(data.train_ds.y, LMLabelList)
        )
...
class LanguageLearner(RNNLearner):
    ...
    def predict(
        ...
        return (
            text + sep + sep.join(decoder(self.data.vocab.textify(new_idx, sep=None)))
        )
    ...
    def beam_search(
        ...
        return (
            ...
            + sep.join(
                decoder(
                    self.data.vocab.textify(
                        [i.item() for i in nodes[node_idx][1:]], sep=None
                    )
                )
            )
        )
...
def get_text_classifier(
    ...
    encoder = MultiBatchEncoder(
        bptt, max_len, arch(vocab_sz, **config), pad_idx=pad_idx
    )
