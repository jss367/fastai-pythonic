...
class ImageDownloader:
    ...
    def _init_ui(self) -> VBox:
        ...
        self._heading = ""
        self._download_complete_heading = (
    ...
    def on_download_button_click(self, btn) -> None:
        ...
        if not self.validate_search_input():
            return
        self.clear_imgs()
        downloaded_images = download_google_images(
            self._path, term, n_images=limit, size=size
        )
    ...
    def display_images_widgets(self, fnames: list) -> None:
        ...
        imgs = [
            widgets.Image(value=open(f, "rb").read(), width="200px") for f in fnames
        ]
...
def download_google_images(
    ...
    if n_images <= 100:
        ...
        img_tuples = _fetch_img_tuples_webdriver(
            search_url, format=format, n_images=n_images
        )
...
def _html_to_img_tuples(html: str, format: str = "jpg", n_images: int = 10) -> list:
    ...
    img_tuples = (
        (_img_fname(d["ou"]), d["ou"]) for d in metadata_dicts if d["ity"] == format
    )
    return list(itertools.islice(img_tuples, n_images))


def _fetch_img_tuples_webdriver(
    url: str, format: str = "jpg", n_images: int = 150
) -> list:
    """
    Parse the Google Images Search for urls and return the image metadata as tuples (fname, url).
...
def _download_single_image(
    label_path: Path, img_tuple: tuple, i: int, timeout: int = 4
) -> None:
    """
