...
class BasicImageWidget(ABC):
    ...
    def make_dropdown_widget(
        ...
        dd = widgets.Dropdown(
            description=description, options=options, value=value, layout=layout
        )
    ...
    def make_horizontal_box(
        children: Collection[widgets.Widget], layout=Layout()
    ) -> widgets.HBox:
...
class ImageCleaner(BasicImageWidget):
        ...
        for i, img in enumerate(batch):
            ...
            if not self._duplicates:
                dropdown = self.make_dropdown_widget(
                    ...
                    value=img.label,
                    handler=self.relabel,
                    img_idx=i,
                )
            delete_btn = self.make_button_widget(
                "Delete", handler=self.on_delete, img_idx=i
            )
            widgets.append(
    ...
    def render(self, batch: Tuple[ImgData]):
        ...
        if self.batch_contains_deleted(batch):
            ...
            display(
                self.make_button_widget(
                    "Next Batch", handler=self.next_batch, style="primary"
                )
            )
