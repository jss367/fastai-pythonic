...
class ClassConfusion:
            ...
            if interp.learn.data.train_ds.x.cont_names != []:
                for x in range(len(interp.learn.data.procs)):
                    if "Normalize" in str(interp.learn.data.procs[x]):
                        self.means = (
                            interp.learn.data.train_ds.x.processor[0].procs[x].means
                        )
                        self.stds = (
                            interp.learn.data.train_ds.x.processor[0].procs[x].stds
                        )
    ...
    def _create_tabs(self):
        ...
        self.lis = (
            self.classl if self.is_ordered else list(permutations(self.classl, 2))
        )
        if self._is_tab:
        ...
        for j, x in enumerate(self.df_list):
            ...
            if tab in self.cat_names:
                ...
                if self._boxes is not None:
                    if vals.nunique() < 10:
                        fig = vals.plot(
                            kind="bar", title=title, ax=ax[j], rot=0, width=0.75
                        )
                    ...
                    fig = vals.plot(
                        kind="barh", title=title, ax=ax[row, col], width=0.75
                    )
            else:
                vals = x[tab]
                if self._boxes is not None:
                    ...
                    axs = vals.plot(
                        kind="hist", ax=ax[row, col], title=title, y="Frequency"
                    )
    ...
    def _tab_losses(self, **kwargs):
        ...
        for j, x in enumerate(comb):
            ...
            for i, idx in enumerate(self.tl_idx):
                ...
                cl = int(cl)
                if (
                    classes[self.interp.pred_class[idx]] == comb[j][0]
                    and classes[cl] == comb[j][1]
                ):
                    res = ""
                    for c, n in zip(da.cats, da.names[: len(da.cats)]):
                        string = f"{da.classes[n][c]}"
            ...
            for i, var in enumerate(self.interp.data.cont_names):
                f[var] = f[var].apply(
                    lambda x: float(x) * self.stds[var] + self.means[var]
                )
            f[str(x)] = str(x)
            self.df_list.append(f)
