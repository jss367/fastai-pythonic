...
def fit_fc(
    ...
    callbacks.append(
        FlatCosAnnealScheduler(
            learn, lr, moms=moms, start_pct=start_pct, tot_epochs=tot_epochs
        )
    )
...
class ShowGraph(LearnerCallback):
    ...
    def on_epoch_end(self, n_epochs: int, last_metrics: MetricsList, **kwargs) -> bool:
        ...
        if last_metrics is not None and last_metrics[0] is not None:
            ...
            x_bounds = (
                0,
                (n_epochs - len(rec.nb_batches)) * rec.nb_batches[-1] + len(rec.losses),
            )
...
class ClassificationInterpretation(Interpretation):
    ...
    def confusion_matrix(self, slice_size: int = 1):
        ...
        if slice_size is None:
            cm = (
                (self.pred_class == x[:, None]) & (self.y_true == x[:, None, None])
            ).sum(2)
        else:
...
class MultiLabelClassificationInterpretation(Interpretation):
    ...
    def __init__(
        ...
        self.pred_class = (
            self.preds.sigmoid(dim=1) > thresh if sigmoid else self.preds > thresh
        )
