...
class TabularProcessor(PreProcessor):
    ...
    def process_one(self, item):
        ...
        if len(self.cat_names) != 0:
            codes = (
                np.stack(
                    [c.cat.codes.values for n, c in df[self.cat_names].items()], 1
                ).astype(np.int64)
                + 1
            )
        else:
    ...
    def process(self, ds):
        if ds.inner_df is None:
            ds.classes, ds.cat_names, ds.cont_names = (
                self.classes,
                self.cat_names,
                self.cont_names,
            )
        ...
        if len(ds.cat_names) != 0:
            ds.codes = (
                np.stack(
                    [c.cat.codes.values for n, c in ds.inner_df[ds.cat_names].items()],
                    1,
                ).astype(np.int64)
                + 1
            )
        ...
        if len(ds.cont_names) != 0:
            ds.conts = np.stack(
                [
                    ...
                    for n, c in ds.inner_df[ds.cont_names].items()
                ],
                1,
            )
