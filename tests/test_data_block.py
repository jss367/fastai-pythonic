...
def test_splitdata_datasets():
    ...
    assert set(list(sd.train.items) + list(sd.valid.items)) == set(
        range(n)
    ), "All items covered"
...
def test_filter_by_rand():
    ...
    assert len(sd1) == len(
        sd2
    ), "Identically seeded random data sets are of different sizes"
    assert (
        sd1.items == sd2.items
...
def test_split_subsets():
    ...
    sd = (
        ItemList(range(10)).split_subsets(train_size=0.1, valid_size=0.2).label_const(0)
    )
...
def test_regression():
    ...
    data = (
        ItemList.from_df(df, path=".", cols=0)
        .split_by_rand_pct()
