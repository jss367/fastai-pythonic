    ...
    for func in ["from_csv", "from_df"]:
        ...
        if func == "from_df":
            data = ImageDataBunch.from_df(
                path, df=pd.read_csv(path / "labels.csv"), size=28
            )
        else:
...
def test_trunc_download():
    ...
    with responses.RequestsMock() as rsps:
        ...
        try:
            ...
            assert (
                False
            ), f"untar_data({URLs.COCO_TINY}) had Unexpected error: {sys.exc_info()[0]}"
        else:
            assert (
                False
...
def test_vision_datasets():
    ...
    assert np.array_equal(
        sds.train.classes, sds.valid.classes
    ), "train/valid classes same"
...
def test_points():
    ...
    data = (
        PointsItemList.from_folder(coco)
        .split_by_rand_pct()
...
def test_vision_pil2tensor_16bit():
    ...
    im = PIL.Image.open(f).convert(
        "I"
    )  # so that the 16bit values are preserved as integers
...
def test_vision_pil2tensor_numpy():
    ...
    diff = np.sort(pil2tensor(arr, np.float).data.numpy().flatten()) - np.sort(
        arr.flatten()
    )
