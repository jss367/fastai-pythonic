...
def test_fuzzy_test_match():
    ...
    assert (
        len(result) == 0
    ), "should only find parent test functions with `def test_` prefix"


def test_fuzzy_line_match():
    this_tests(nbtest._fuzzy_line_match)
    # Testing _fuzzy_test_match private methods
    result = nbtest._fuzzy_line_match(
        "Databunch.get", ["d = DataBunch()", "item = d.get(5)"]
    )
    assert len(result) == 1, "finds class methods"

    result = nbtest._fuzzy_line_match(
        "TextList", ["tl = (TextList.from_df()", "", "LMTextList()"]
    )
...
def test_get_tests_dir():
    ...
    assert (
        result.parts[-1] == "tests"
    ), f"Failed: get_tests_dir return unexpected result: {result}"
