...
def test_forget_mult_forward_gpu():
    ...
    x, f, expected = [
        torch.tensor(t, dtype=dtype)[None, :, None].cuda() for t in (x, f, expected)
    ]
...
def test_forget_mult_cuda():
    ...
    for (bf, bw) in [(True, True), (False, True), (True, False), (False, False)]:
        ...
        th_out = forget_mult_CPU(
            th_x, th_f, hidden_init=None, batch_first=bf, backward=bw
        )
        ...
        th_out = forget_mult_CPU(
            th_x, th_f, hidden_init=th_h, batch_first=bf, backward=bw
        )
        th_loss = th_out.pow(2).mean()
        th_loss.backward()
...
@pytest.mark.skipif(
    not have_min_pkg_version("torch", "1.0.2"), reason="requires torch>=1.0.2"
)
def test_qrnn_layer():
    this_tests(QRNNLayer)
    qrnn_fwd = QRNNLayer(
        10, 20, save_prev_x=True, zoneout=0, window=2, output_gate=True
    )
    qrnn_bwd = QRNNLayer(
...
def test_qrnn_bidir():
    this_tests(QRNN)
    qrnn = QRNN(
        10, 20, 2, bidirectional=True, batch_first=True, window=2, output_gate=False
    )
