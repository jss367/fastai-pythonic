...
def config(qrnn: bool = False):
    ...
    config["emb_sz"], config["n_hid"], config["n_layers"], config["qrnn"] = (
        100,
        100,
        1,
        qrnn,
...
def learn():
    ...
    learn = language_model_learner(
        data, AWD_LSTM, pretrained=False, config=config(), drop_mult=0.0
    )
...
def test_qrnn_works_with_no_split():
    ...
    data = TextLMDataBunch.from_df(
        path, df_trn, df_val, tokenizer=Tokenizer(BaseTokenizer)
    )
    learn = language_model_learner(
    ...
    for n_labels in [1, 8]:
        ...
        try:
            ...
            classifier = text_classifier_learner(
                data, AWD_LSTM, pretrained=False, bptt=10
            )
            assert last_layer(classifier.model).out_features == expected_classes
            assert len(data.train_dl) == math.ceil(
                len(data.train_ds) / data.train_dl.batch_size
            )
...
def test_mem_leak():
    ...
    gc_collected = (
        gc.collect()
    )  # should be 0 too - !0 means we have circular references
    assert (
        gc_collected < 102
