...
def test_apply_init():
    ...
    assert bn1_w.equal(m[0][2].weight), (
        "Expected first batch norm layers weights to be %r" % bn1_w
    )
...
def test_to_detach():
    ...
    assert isinstance(
        dtta,
        (torch.cuda.FloatTensor or torch.cuda.DoubleTensor or torch.cuda.HalfTensor),
    )
    assert not dtta.requires_grad
    assert not dttacpu.is_cuda
    assert isinstance(
        dttacpu, (torch.FloatTensor or torch.DoubleTensor or torch.HalfTensor)
    )
...
def test_to_cpu():
    ...
    assert ta.is_cuda
    assert isinstance(
        ta, (torch.cuda.LongTensor or torch.cuda.IntTensor or torch.cuda.ShortTensor)
    )
...
def test_none_reduce_on_cpu():
    ...
    with NoneReduceOnCPU(nn.CrossEntropyLoss()) as lf:
        ...
        ), "final loss does not seem to be correct"
...
def test_tensor_array_monkey_patch():
    ...
    assert np.all(
        t == t
    ), "Tensors did not properly convert to numpy arrays with a dtype set"
