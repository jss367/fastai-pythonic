...
def test_random_split():
    ...
    a = [
        len(arr)
        for arr in random_split(valid_pct, [1, 2, 3, 4, 5], ["a", "b", "c", "d", "e"])
    ]
    b = [2, 2]
    assert a == b
...
def test_join_paths():
    ...
    assert set(join_paths({"f1", "f2"}, Path("dir"))) == {
        Path("dir/f1"),
        Path("dir/f2"),
    }
...
def test_itembase_hash():
    this_tests(
        ItemBase.__eq__, Category.__hash__, FloatItem.__hash__, MultiCategory.__hash__
    )
