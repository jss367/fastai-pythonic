...
def test_gpu_mem_trace_decorator():
    ...
    check_mem(
        used_exp=0, peaked_exp=0, used_rcv=used, peaked_rcv=peaked, abs_tol=2, ctx=""
    )
...
def reduce_mem_usage(df):
    ...
    for col in columns:
        ...
        if (
            str(col_type) != "category"
            and col_type != "datetime64[ns]"
            and col_type != bool
        ):
            if col_type != object:
                c_min = df[col].min()
                ...
                if str(col_type)[:3] == "int":
                    ...
                    if (
                        ...
                        print(
                            "Error " + col + " value would be a float64. Disregarding."
                        )
            else:
