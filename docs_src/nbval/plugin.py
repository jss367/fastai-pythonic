...
class IPyNbFile(pytest.File):
    ...
    def setup(self):
        ...
        if getattr(self.parent.config.option, "cov_source", None):
            setup_coverage(
                self.parent.config, self.kernel, getattr(self, "fspath", None)
            )
...
class IPyNbCell(pytest.Item):
    ...
    def reportinfo(self):
        description = "%s::Cell %d" % (
            self.fspath.relto(self.config.rootdir),
            self.cell_num,
        )
                ...
                if key not in skip_compare:
                    if key == "data":
                        for data_key in testing[key].keys():
                            if data_key not in skip_compare:
                                testing_outs[data_key].append(
                                    self.sanitize(testing[key][data_key])
                                )
                    else:
                    ...
                    if key == "data":
                        ...
                        for data_key in output[key].keys():
                            ...
                            if data_key in skip_sanitize:
                                ...
                                sanitized[key][data_key] = self.sanitize(
                                    output[key][data_key]
                                )
