...
def start_new_kernel(startup_timeout=60, kernel_name="python", **kwargs):
    ...
    km = KernelManager(
        kernel_name=kernel_name, kernel_spec_manager=NbvalKernelspecManager()
    )
    ...
    def _ensure_iopub_up(self):
        ...
        for _ in range(total_timeout // individual_timeout):
            ...
            try:
                ...
                raise RuntimeError(
                    "Kernel info reqest timed out after %d seconds!" % shell_timeout
                )
            ...
            raise RuntimeError(
                "Wasn't able to establish IOPub after %d seconds." % total_timeout
            )
