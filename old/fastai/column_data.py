...
class ColumnarDataset(Dataset):
    ...
    def __init__(self, cats, conts, y, is_reg, is_multi):
        ...
        self.conts = (
            np.stack(conts, 1).astype(np.float32) if conts else np.zeros((n, 1))
        )
...
class ColumnarModelData(ModelData):
    def __init__(self, path, trn_ds, val_ds, bs, test_ds=None, shuffle=True):
        test_dl = (
            DataLoader(test_ds, bs, shuffle=False, num_workers=1)
    ...
    def from_data_frames(
        ...
        trn_ds = ColumnarDataset.from_data_frame(
            trn_df, cat_flds, trn_y, is_reg, is_multi
        )
        val_ds = ColumnarDataset.from_data_frame(
...
class CollabFilterDataset(Dataset):
    def __init__(self, path, user_col, item_col, ratings):
        ...
        self.n = len(ratings)
        (self.users, self.user2idx, self.user_col, self.n_users) = self.proc_col(
            user_col
        )
        (self.items, self.item2idx, self.item_col, self.n_items) = self.proc_col(
            item_col
        )
