...
class RandomScale(CoordTransform):
    ...
    def set_state(self):
        ...
        self.store.mult = (
            random.uniform(min_z, max_z) if random.random() < self.p else 1
        )
...
class RandomRotateZoom(CoordTransform):
    ...
    def __init__(
        self, deg, zoom, stretch, ps=None, mode=cv2.BORDER_REFLECT, tfm_y=TfmType.NO
    ):
        super().__init__(tfm_y)
        if ps is None:
...
class RandomZoom(CoordTransform):
    ...
    def set_state(self):
        self.store.zoom = (
            self.zoom_min + (self.zoom_max - self.zoom_min) * random.random()
        )
...
def tfms_from_stats(
    ...
    if aug_tfms is None:
        aug_tfms = []
    tfm_norm = (
        Normalize(*stats, tfm_y=tfm_y if norm_y else TfmType.NO)
    ...
    val_crop = (
        CropType.CENTER
        if crop_type in (CropType.RANDOM, CropType.GOOGLENET)
        else crop_type
    )
    val_tfm = image_gen(
        tfm_norm,
