...
class CircularLR(LR_Updater):
    ...
    def __init__(
        self, layer_opt, nb, div=4, cut_div=8, on_cycle_end=None, momentums=None
    ):
        self.nb, self.div, self.cut_div, self.on_cycle_end = (
            nb,
            div,
            cut_div,
            on_cycle_end,
...
class SaveBestModel(LossRecorder):
    ...
    def __init__(self, model, layer_opt, metrics, name="best_model"):
        ...
        self.best_acc = None
        self.save_method = (
            self.save_when_only_loss if metrics == None else self.save_when_acc
        )
...
class WeightDecaySchedule(Callback):
    def __init__(
        ...
        self.new_wds = (
            None  # Holds the new weight decay factors, calculated in on_batch_begin()
        )
        self.iteration = 0
        self.epoch = 0
