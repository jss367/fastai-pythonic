    ...
    if resume:
        ...
        for subdir in subdirs:
            files = [
                ...
                for file in os.listdir(os.path.join(target_path, subdir))
            ]
...
def csv_source(
    folder, csv_file, skip_header=True, suffix="", continuous=False, cat_separator=" "
):
...
def dict_source(folder, fnames, csv_labels, suffix="", continuous=False):
    all_labels = sorted(
        list(
            set(p for o in csv_labels.values() for p in ([] if type(o) == float else o))
        )
    )
...
class ModelData:
    ...
    def __init__(self, path, trn_dl, val_dl, test_dl=None):
        self.path, self.trn_dl, self.val_dl, self.test_dl = (
            path,
            trn_dl,
            val_dl,
            test_dl,
...
class ImageData(ModelData):
    def __init__(self, path, datasets, bs, num_workers, classes):
        ...
        self.path, self.bs, self.num_workers, self.classes = (
            path,
            bs,
            num_workers,
            classes,
    ...
    def resized(self, dl, targ, new_path, resume=True, fn=None):
        ...
        """
        return (
            dl.dataset.resize_imgs(targ, new_path, resume=resume, fn=fn) if dl else None
        )
...
class ImageClassifierData(ImageData):
    ...
    def from_paths(
        ...
        if test_name:
            test = (
                folder_source(path, test_name)
                if test_with_labels
                else read_dir(path, test_name)
            )
        else:
            test = None
        datasets = cls.get_ds(
    ...
    def from_path_and_array(
        ...
        assert not (os.path.isabs(folder)), "folder needs to be a relative path"
        fnames = np.core.defchararray.add(
            f"{folder}/", sorted(os.listdir(f"{path}{folder}"))
        )
        return cls.from_names_and_array(
            path,
            fnames,
