...
def parse_module(file_path):
    ...
    for cls_def in cls_defs:
        ...
        tmpl_str += (
            f'== {{{{class {cls_name}{":" + cls_bases if cls_bases else ""}}}}}\n\n'
        )
        method_str = None
        for fn_def in (
            fn_def for fn_def in cls_def.body if isinstance(fn_def, ast.FunctionDef)
        ):
            if fn_def.name == "__init__":
                ...
                method_str += (
                    f'{{{{method {fn_def.name},{doc_str if doc_str else ""}}}}}\n\n'
                )
...
def gen_ascii_docs(src="fastai"):
    ...
    for file_path in file_paths:
        if pat.match(file_path.name):
            ...
            (
                file_path.parent / (file_path.name.rsplit(".", 1)[0] + ".adoc.tmpl")
            ).write_text(tmpl_str)
            (
                file_path.parent / (file_path.name.rsplit(".", 1)[0] + ".adoc")
            ).write_text(re.sub(r"{{(.*?)}}", parse_tmpl, tmpl_str, flags=re.DOTALL))
    if path.is_dir():
